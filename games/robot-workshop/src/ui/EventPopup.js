...
export function createEventPopup({ layout, assets, width, height, pause = () => false, resume = () => {} }) {
  ...
  function measure(ctx, o) {
    ...
    const btnH = buttons.map((b) => (b.sub ? 132 : 104));
  ...
  const popup = {
    ...
    render(ctx) {
      ...
      const accent = o.accent ?? '#4FC3F7';
      const lift = (1 - k) * 40;
      panel(ctx, { x: L.x, y: L.y + lift, w: L.w, h: L.h }, { fill: 'rgba(22,28,36,0.98)', stroke: accent, lineWidth: 6, radius: 32 });
      let ty;
      if (o.art) {
      ...
      L.bodyLines.forEach((line, i) => text(ctx, line, L.x + 40, by + i * 42, { size: 32, color: '#E8EEF2', maxWidth: L.w - 80 }));
      const ready = cur.t >= MIN_SHOW;
      L.buttons.forEach((b, i) => {
        const r = { ...L.rects[i], y: L.rects[i].y + lift };
        drawButton(ctx, r, '', { accent: i === 0 ? accent : '#9AA8B5', disabled: !ready });
        text(ctx, b.label, r.x + r.w / 2, r.y + (b.sub ? 40 : r.h / 2 - 3), { size: 36, bold: true, align: 'center', baseline: 'middle', maxWidth: r.w - 40 });
        if (b.sub) text(ctx, b.sub, r.x + r.w / 2, r.y + 88, { size: 25, align: 'center', baseline: 'middle', color: '#C9D3DD', maxWidth: r.w - 48 });
      });
      ctx.restore();
    },
  };
