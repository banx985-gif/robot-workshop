...
export function createProjectDetailScreen({ renderer, layout, assets, campaign, router, hud }) {
  ...
  const focusRect = (i) => {
    const w = (cw() - 32) / 3;
    return { x: i * (w + 16), y: Y.budget + 56, w, h: 96 };
  };
  ...
  const screen = {
    ...
    render(ctx) {
      ...
      text(ctx, `${purpose.name} · ${tier.name} tier`, 210, 88, { size: 28, color: '#9AA8B5', maxWidth: w - 230 });
      text(ctx, `Worked ${j.day} days · running cost ${fmt(campaign.operatingCostPerDay(j))} credits a day`, 210, 134, { size: 26, color: '#9AA8B5', maxWidth: w - 230 });
      ...
      PHASES.forEach((ph, i) => {
        ...
        if (i > 0) {
          ctx.fillStyle = i <= j.phaseIndex ? '#7CFFB2' : '#35414F';
          ctx.fillRect(cx - stepW, Y.steps + 36, stepW, 8);
        }
        ctx.beginPath();
        ctx.arc(cx, Y.steps + 40, 32, 0, Math.PI * 2);
        ctx.fillStyle = done ? '#7CFFB2' : now ? '#FFD166' : '#2A3440';
        ctx.fill();
        text(ctx, done ? '✓' : String(i + 1), cx, Y.steps + 41, { size: 32, bold: true, color: done || now ? '#101418' : '#9AA8B5', align: 'center', baseline: 'middle' });
        text(ctx, SHORT[ph.id], cx, Y.steps + 84, { size: 24, bold: now, color: now ? '#FFFFFF' : '#9AA8B5', align: 'center' });
      });
      ...
      bar(ctx, 24, Y.phase + 80, w - 48, 44, j.phaseProgress / j.phaseTarget, '#7CFFB2');
      text(ctx, `${fmt(Math.min(j.phaseProgress, j.phaseTarget))} / ${fmt(j.phaseTarget)}`, w / 2, Y.phase + 102, { size: 28, bold: true, align: 'center', baseline: 'middle', color: '#FFFFFF' });
      if (perDay > 0) {
        text(ctx, `+${perDay.toFixed(1)} work per day · about ${left} days left`, 24, Y.phase + 144, { size: 28, color: '#E8EEF2', maxWidth: w - 48 });
      } else {
        text(ctx, 'Nobody is assigned — work has stopped', 24, Y.phase + 144, { size: 30, bold: true, color: '#FF8A80' });
      }
      ...
      text(ctx, rollText, 24, Y.phase + 192, { size: 26, color: roll?.hit ? '#FFD166' : '#9AA8B5', maxWidth: w - 48 });
      ...
      BUDGET_ORDER.forEach((id, i) => {
        ...
        drawButton(ctx, focusRect(i), label, { active: d.budgetFocus === id, accent: d.pendingFocus === id ? '#FFD166' : '#4FC3F7', font: 'bold 30px system-ui, sans-serif' });
      });
      ...
      text(ctx, note, 4, Y.budget + 170, { size: 24, color: d.pendingFocus ? '#FFD166' : '#9AA8B5', maxWidth: w });
      ...
      text(ctx, `Open faults: ${d.faults.length}`, 24, Y.faults + 20, { size: 40, bold: true, color: d.faults.length ? '#FF8A80' : '#7CFFB2' });
      text(ctx, `Found ${d.faultsFound} · fixed ${d.faultsFixed} · each open fault: −3 REL, −1.5 Quality`, 24, Y.faults + 76, { size: 26, color: '#9AA8B5', maxWidth: w - 48 });
      text(ctx, 'Testing & Tuning tries to fix open faults at the end', 24, Y.faults + 116, { size: 26, color: '#9AA8B5', maxWidth: w - 48 });
      ...
      if (!d.breakthroughs.length) text(ctx, 'None yet', 4, Y.log + 50, { size: 26, color: '#9AA8B5' });
      d.breakthroughs.forEach((b, i) => {
        const name = PHASES.find((p) => p.id === b.phase).name;
        text(ctx, `${name} (day ${b.day}): ${b.hit ? 'BREAKTHROUGH' : 'no breakthrough'}`, 4, Y.log + 50 + i * 40, { size: 26, color: b.hit ? '#FFD166' : '#9AA8B5' });
      });
