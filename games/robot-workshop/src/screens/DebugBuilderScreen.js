...
export function createDebugBuilderScreen({ renderer, layout, assets, campaign, router, makeSandbox }) {
  ...
  const screen = {
    ...
    render(ctx) {
      ...
      text(ctx, 'any purpose, any parts — locks ignored · the real save is not touched', sr.x + 230, sr.y + 98, { size: 24, color: COL.action, baseline: 'middle', maxWidth: sr.w - 260 });

      const w = cw();
      scroll.begin(ctx);
      text(ctx, 'Purpose', 4, Y.purpose, { size: 30, bold: true });
      PURPOSE_ORDER.forEach((id, i) => drawButton(ctx, purposeRect(i), PURPOSES[id].shortName, { selected: state.purposeId === id, font: font(26, true) }));
      ...
      SLOTS.forEach((s, si) => {
        ...
        partsInSlot(s.id).forEach((c, i) => {
          ...
          panel(ctx, r, { fill: on ? 'rgba(79,195,247,0.25)' : COL.panel, stroke: on ? COL.progress : COL.line, lineWidth: on ? 5 : 2, radius: 14 });
          contained(ctx, assets, c.art, { x: r.x + 6, y: r.y + 6, w: r.w - 12, h: r.h - 12 });
          if (!campaign.partOpen(c.id, { ignoreDebug: true })) drawPadlock(ctx, r.x + r.w - 24, r.y + r.h - 18, 18, COL.gold);
        });
      });

      text(ctx, 'Budget focus', 4, Y.focus, { size: 30, bold: true });
      BUDGET_ORDER.forEach((id, i) => drawButton(ctx, focusRect(i), BUDGET_FOCUS[id].name, { selected: state.focus === id, font: font(30, true) }));
      ...
      ACTIONS.forEach((a, i) =>
        drawButton(ctx, actionRect(i), labels[a], {
          ...
          accent: a === 'build' ? COL.good : a === 'unlock' ? COL.action : COL.progress,
          font: font(28, true),
        }),
      );
  ...
  function drawResult(ctx, w) {
    const y = Y.result;
    panel(ctx, { x: 0, y, w, h: RESULT_H }, { stroke: last?.problems.length ? COL.bad : COL.line });
    ...
    if (!last) {
      text(ctx, 'Press "Build now" to run the whole project instantly.', 24, y + 80, { size: 30, color: COL.textMuted, maxWidth: w - 48 });
      return;
    }
    ...
    text(ctx, `${PURPOSES[r.purpose].name} · visual ${r.visual} ${fam?.name ?? '?'}`, 290, y + 130, { size: 26, color: COL.textMuted, maxWidth: w - 310 });
    text(ctx, `${t.name} tier · complexity ${r.totalCx} · cost ${fmt(r.buildCost)}`, 290, y + 170, { size: 26, color: COL.textMuted, maxWidth: w - 310 });
    text(ctx, `${last.days} work days (≈${(last.days / 28).toFixed(1)} months) · ran in ${last.ms.toFixed(0)} ms`, 290, y + 210, { size: 26, color: COL.textMuted, maxWidth: w - 310 });
    text(ctx, `Quality ${r.quality.toFixed(1)}`, 290, y + 256, { size: 44, bold: true, color: COL.good });
    text(ctx, `Review ${r.review.toFixed(1)} / 10`, 290, y + 310, { size: 36, bold: true, color: COL.gold });
    statBars(ctx, 24, y + 370, w - 48, r.stats, ROBOT_STATS, { color: COL.progress, scaleMax: 600 });
    ...
    text(ctx, `Faults left ${r.faults} (found ${r.faultsFound}, fixed ${r.faultsFixed}) · budget ${r.budgetFocus}`, 24, y2 + 46, { size: 26, color: COL.text, maxWidth: w - 48 });
    text(ctx, SLOTS.map((s) => r.components[s.id]).join(' · '), 24, y2 + 88, { size: 26, color: COL.textMuted, maxWidth: w - 48 });
    const ok = !last.problems.length;
    text(ctx, ok ? 'Number check: all values are real numbers ✓' : `Problems: ${last.problems.join(', ')}`, 24, y2 + 134, { size: 28, bold: true, color: ok ? COL.good : COL.bad, maxWidth: w - 48 });
  }
