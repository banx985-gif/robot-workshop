...
const GOLD = COL.gold;
const GREY = COL.textFaint;
const RED = COL.bad;
const STATUS_COLOR = { done: GREEN, active: CYAN, available: COL.text, locked: GREY };
...
export function createResearchScreen({ renderer, layout, assets, bus, campaign, router, debugEnabled = false, ads = null }) {
  ...
  const rowButton = (i) => {
    const r = rowRect(i);
    return { x: r.x + r.w - 20 - 230, y: r.y + r.h - 20 - 110, w: 230, h: 110 };
  };
  ...
  const debugRect = () => ({ x: 0, y: milestoneTop() + 70 + RESEARCH_MILESTONES.length * 64 + 10, w: 420, h: 110 });
  ...
  const screen = {
    ...
    render(ctx) {
      ...
      if (debugEnabled) drawButton(ctx, debugRect(), 'Debug: finish all', { font: font(30, true), accent: COL.bad });
      scroll.end(ctx);
  ...
  function drawQueue(ctx, i) {
    ...
    panel(ctx, r, { fill: open ? COL.panel : COL.panelDim, stroke: node ? CYAN : open ? COL.line : COL.line, lineWidth: node ? 5 : 3 });
    text(ctx, `Queue ${i + 1}`, r.x + 24, r.y + 18, { size: 26, bold: true, color: COL.textMuted });
    if (!open) {
      drawPadlock(ctx, r.x + 34, r.y + 92, 30, COL.action);
      const needs = missingParts(def.rule, (rule) => campaign.ruleMet(rule)).join(' + ');
      text(ctx, i === 0 ? 'Build a Research Desk to open research' : 'Locked', r.x + 74, r.y + 70, { size: 32, bold: true, color: COL.textMuted, maxWidth: r.w - 300 });
      text(ctx, `Needs ${needs}`, r.x + 74, r.y + 110, { size: 26, color: COL.action, maxWidth: r.w - 300 });
      if (i === 0) drawButton(ctx, { x: r.x + r.w - 20 - 200, y: r.y + r.h / 2 - 55, w: 200, h: 110 }, 'Build', { accent: COL.action, font: font(32, true) });
      return;
    }
    if (!node) {
      text(ctx, 'Free — pick a topic below', r.x + 24, r.y + 64, { size: 34, bold: true, maxWidth: r.w - 48 });
      text(ctx, `${res.nodes.filter((n) => res.status(n.id) === 'available').length} topics available · ${res.doneCount}/36 done`, r.x + 24, r.y + 108, { size: 26, color: COL.textMuted, maxWidth: r.w - 48 });
      return;
    }
  ...
  function drawMap(ctx) {
    for (const b of branches()) {
      ...
      const sel = b === branch;
      panel(ctx, r, { fill: sel ? COL.panelInfo : COL.panelAlt, stroke: sel ? info.color : COL.line, lineWidth: sel ? 5 : 3, radius: 18 });
      text(ctx, info.short, r.x + r.w / 2, r.y + 14, { size: 26, bold: true, align: 'center', color: sel ? info.color : COL.textMuted, maxWidth: r.w - 10 });
      const done = list.filter((n) => res.isDone(n.id)).length;
      text(ctx, `${done}/${list.length}`, r.x + r.w / 2, r.y + 46, { size: 22, align: 'center', color: COL.textMuted });
      const cx = r.x + r.w / 2;
      const top = r.y + 98;
      ...
      list.forEach((n, k) => {
        ...
        ctx.fillStyle = st === 'locked' ? COL.panelDim : STATUS_COLOR[st];
        ctx.fill();
  ...
  function drawNode(ctx, n, i, list) {
    ...
    if (i < list.length - 1) {
      ctx.fillStyle = res.isDone(n.id) ? GREEN : COL.line;
      ctx.fillRect(r.x + 52, r.y + r.h, 6, GAP);
    }
    panel(ctx, r, { fill: st === 'locked' ? COL.panelDim : COL.panel, stroke: st === 'available' ? info.color : st === 'active' ? CYAN : st === 'done' ? COL.panelGood : COL.line, lineWidth: st === 'available' || st === 'active' ? 5 : 3 });
    ctx.beginPath();
    ctx.arc(r.x + 55, r.y + 56, 34, 0, Math.PI * 2);
    ctx.fillStyle = st === 'locked' ? COL.panelDim : st === 'done' ? COL.panelGood : COL.panelAlt;
    ctx.fill();
    ctx.strokeStyle = color;
    ctx.lineWidth = 4;
    ctx.stroke();
    text(ctx, st === 'done' ? '✓' : String(n.level), r.x + 55, r.y + 57, { size: 34, bold: true, align: 'center', baseline: 'middle', color });
    if (st === 'locked') drawPadlock(ctx, r.x + 42, r.y + 118, 26, COL.textFaint);

    const x = r.x + 110;
    const mw = r.w - 130;
    text(ctx, secretHidden(n) ? '??? · A secret topic' : `${n.id} · ${n.name}`, x, r.y + 16, { size: 32, bold: true, color: st === 'locked' ? COL.textMuted : COL.text, maxWidth: mw - 220 });
    ...
    text(ctx, right, r.x + r.w - 20, r.y + 18, { size: 30, bold: true, align: 'right', color: st === 'done' ? GREEN : st === 'active' ? CYAN : st === 'locked' ? COL.textFaint : res.rp >= cost || res.paid[n.id] ? GOLD : RED });
    ...
    for (const u of secretHidden(n) ? [] : unlockItems(n)) {
      ...
      text(ctx, label, ux, uy + 30, { size: 24, bold: true, baseline: 'middle', color: st === 'locked' ? COL.textFaint : GREEN, maxWidth: w });
      ux += w + 24;
    }
    ...
    text(ctx, line3, x, r.y + 132, { size: 24, color: opens.length ? COL.textMuted : COL.textMuted, maxWidth: st === 'available' ? mw - 260 : mw });
    ...
    if (st === 'locked') {
      ...
      text(ctx, `Needs ${needs.join(' + ')}`, x, r.y + 170, { size: 25, bold: true, color: COL.action, maxWidth: mw });
    } else if (st === 'active') {
      ...
      drawButton(ctx, rowButton(i), a.label, { font: font(30, true), disabled: !a.ok, accent: info.color });
    }
  }
  ...
  function drawMilestones(ctx, y) {
    ...
    RESEARCH_MILESTONES.forEach((m, k) => {
      ...
      const ry = y + 64 + k * 64;
      panel(ctx, { x: 0, y: ry, w, h: 54 }, { fill: hitIt ? COL.panelGood : COL.panelAlt, stroke: hitIt ? COL.panelGood : COL.line, radius: 14 });
      ...
      text(ctx, hitIt ? `✓ ${names} — ${note}` : names, 90, ry + 27, { size: 24, baseline: 'middle', color: hitIt ? GREEN : COL.textMuted, maxWidth: w - 110 });
    });
  }
  ...
  function drawPicker(ctx) {
    ...
    panel(ctx, d, { fill: COL.panel, stroke: info.color, lineWidth: 5, radius: 28 });
    ...
    text(ctx, 'They leave the robot team while researching.', d.x + 30, d.y + 246, { size: 24, color: COL.textMuted, maxWidth: d.w - 60 });
    campaign.staff.staff.forEach((s, i) => {
      ...
      text(ctx, `${ROLES[s.role].name} · ${stat.short} ${statOf(s, node)}`, r.x + 104, r.y + 58, { size: 24, color: COL.textMuted, maxWidth: r.w - 420 });
      ctx.restore();
      const tag = block ?? `≈ ${daysWith(node, s)} days`;
      text(ctx, on ? `✓ ${tag}` : tag, r.x + r.w - 20, r.y + r.h / 2, { size: 26, bold: true, align: 'right', baseline: 'middle', color: block ? RED : on ? GREEN : COL.textMuted, maxWidth: 300 });
    });
    ...
    drawButton(ctx, dialogButton(0), label, { active: ok, disabled: !ok, accent: GREEN, font: font(36, true) });
